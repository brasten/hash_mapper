...
describe 'mapping a hash with one level, `from` only' do
  ...
  it "should map to" do
    OneLevelWithFromOnly.normalize(@from).should == @to
  end

  it "should have indifferent access" do
    OneLevelWithFromOnly.normalize({'name' => 'ismael'}).should == @to
  end

  it "should map back the other way" do
    OneLevelWithFromOnly.denormalize(@to).should == @from
  end
